...
// which are part of the Ttm repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="ttm-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /ttm-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "ttmpay-ttm/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from ttmpay-ttm/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'ttmpay-ttm/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from ttmpay-ttm/develop"
            }
          }

          if (hasCache) {
            sh "cd /ttm-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /ttm-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /ttm-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /ttm-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /ttm-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /ttm-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /ttm-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
