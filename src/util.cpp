...
const char * const BITCOIN_CONF_FILENAME = "ttm.conf";
const char * const BITCOIN_PID_FILENAME = "ttmd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TtmCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TtmCore
    // Mac: ~/Library/Application Support/TtmCore
    // Unix: ~/.ttmcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TtmCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TtmCore";
#else
    // Unix
    return pathRet / ".ttmcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ttm.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ttm.conf
